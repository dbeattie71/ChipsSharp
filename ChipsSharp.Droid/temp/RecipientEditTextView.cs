...
namespace com.dbeattie
{
	...
	public class RecipientEditTextView : MultiAutoCompleteTextView,
		AdapterView.IOnItemClickListener,
		GestureDetector.IOnGestureListener,
		TextView.IOnEditorActionListener
	{
		private static char COMMIT_CHAR_COMMA = ',';

		private static char COMMIT_CHAR_SEMICOLON = ';';

		private static char COMMIT_CHAR_SPACE = ' ';

		private static string SEPARATOR = COMMIT_CHAR_COMMA + COMMIT_CHAR_SPACE.ToString();

		private static string TAG = "RecipientEditTextView";
     ...
     */
		...
		private List<DrawableRecipientChip> mRemovedSpans;
	...
	*/
		...
		public RecipientEditTextView(Context context, IAttributeSet attrs) : base(context, attrs)
		{
			mAddTextWatcher = () =>
			{
				if (mTextWatcher == null)
...
//			outAttrs.ActionLabel = Context.GetString(Resource.String.done);
		...
		public override void Append(ICharSequence text, int start, int end)
		{
			...
			if (!TextUtils.IsEmpty(text) && TextUtils.GetTrimmedLength(text) > 0)
			{
				...
				if (!displayString.Trim().EndsWith(COMMIT_CHAR_COMMA.ToString()))
				{
					...
					base.Append(SEPARATOR, 0, SEPARATOR.Length);
		...
		private void shrink()
		{
			...
			else
			{
				...
				else
				{
					...
					DrawableRecipientChip[] chips = (DrawableRecipientChip[]) getSpannable().GetSpans(start, end, Class.FromType(typeof(DrawableRecipientChip)));
					if ((chips == null || chips.Length == 0))
					{
     ...
     */
		...
		{
			bool isRtl = Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1
     ...
     */
		...
		internal DrawableRecipientChip constructChipSpan(RecipientEntry contact, bool pressed,
		                                                bool leaveIconSpace)
		{
     ...
     */
		...
		private void setChipHeight(int height)
		{
			mChipHeight = height;
		}
     ...
     */
		...
		private void checkChipWidths()
		{
			...
			{
				Rect bounds;
				for (DrawableRecipientChip chip :
				chips)
				{
...
// Visible for testing.
     ...
     */
		...
		public override void SetTokenizer(ITokenizer tokenizer)
		{
			mTokenizer = tokenizer;
			base.SetTokenizer(tokenizer);
		}
     ...
     */
		...
		private bool commitChip(int start, int end, IEditable editable)
		{
		...
		private bool isCompletedToken(CharSequence text)
		{
			if (TextUtils.isEmpty(text))
			{
				return false;
		...
		private void showAlternates(DrawableRecipientChip currentChip, ListPopupWindow alternatesPopup, int width)
		{
...
// Visible for testing.
		...
		private String createAddressText(RecipientEntry entry)
		{
			...
			return mTokenizer != null && !TextUtils.isEmpty(trimmedDisplayText)
			       && index < trimmedDisplayText.length() - 1
				? (String) mTokenizer
					.terminateToken(trimmedDisplayText)
				: trimmedDisplayText;
		}
...
// Visible for testing.
		...
		private CharSequence createChip(RecipientEntry entry, bool pressed)
		{
...
// Visible for testing.
     ...
     */
		...
		private bool shouldShowEditableText(DrawableRecipientChip currentChip)
		{
		...
		private void showAddress(final DrawableRecipientChip 
		private currentChip 
	,
		private final ListPopupWindow 
		private popup 
	,
		private int width 
	)
     ...
     */
		...
		private bool isInDelete(DrawableRecipientChip chip, int offset, float x, float y)
		{
...
/**
     ...
     */
		...
		private void removeChip(DrawableRecipientChip chip)
		{
			...
			bool wasSelected = chip == mSelectedChip;
...
/**
     ...
     */
		...
		public bool lastCharacterIsCommitCharacter(CharSequence s)
		{
			char last;
     ...
     */
		...
		private ArrayList<DrawableRecipientChip> handlePaste()
		{
...
//		}
		...
		private void processReplacements(List<DrawableRecipientChip> recipients,
...
/**
     ...
     */
		// @Override
		public bool onDragEvent(DragEvent 
		private event
	...
	)
		{
			...
			getAction())
			{
				...
				return event.
				getClipDescription().hasMimeType(ClipDescription.MIMETYPE_TEXT_PLAIN);
				case DragEvent.ACTION_DRAG_ENTERED:
				requestFocus();
				return true;
				case DragEvent.ACTION_DROP:
				handlePasteClip(event.
				getClipData())
				;
				return true;
			}
