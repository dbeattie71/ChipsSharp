...
namespace com.dbeattie
{
    ...
    private static  String TAG = "RecipAlternates";
    ...
    private static  Dictionary<String, String> sCorrectedPhotoUris = new Dictionary<String, String>();
    ...
    public static void getMatchingRecipients(Context context, BaseRecipientAdapter adapter,
        ...
        // directories
         Set<String> matchesNotFound = new HashSet<String>();
     ...
     */
    static RecipientEntry getBetterRecipient( RecipientEntry entry1,
             RecipientEntry entry2) {
    ...
    private static Cursor doQuery(CharSequence constraint, int limit, Long directoryId,
        ...
        if (account != null) {
         ...
         Cursor cursor = resolver.query(builder.build(), query.getProjection(), null, null,
                null);
        return cursor;
    }
        ...
        if (queryType == QUERY_TYPE_EMAIL) {
            ...
            if (directoryId == null || lookupKey == null) {
                ...
                desiredMimeType = null;
                 ...
                 Uri.Builder builder = Contacts.getLookupUri(contactId, lookupKey).buildUpon();
         ...
         Cursor resultCursor = removeUndesiredDestinations(cursor, desiredMimeType, lookupKey);
     ...
     */
    // Visible for testing
    static Cursor removeUndesiredDestinations( Cursor original,  String desiredMimeType,
             String lookupKey) {
        if (original == null) {
         ...
         HashSet<String> destinationsSeen = new HashSet<String>();
        ...
        while (original.moveToNext()) {
             String mimeType = original.getString(Query.MIME_TYPE);
        ...
        while (original.moveToNext()) {
            if (desiredMimeType != null) {
             ...
             String destination = original.getString(Query.DESTINATION);
            ...
            // Ensure we don't have two '?' like content://.../...?account_name=...?sz=...
             String photoThumbnailUri = (String) row[Query.PHOTO_THUMBNAIL_URI];
